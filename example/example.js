...
angular.module('example', ['ui.mention']).run(["$rootScope", function ($rootScope) {
  ...
  return {
    ...
    link: function link($scope, $element, $attrs, uiMention) {
       ...
       */
      uiMention.setConfig([{
        findChoices: function findChoices(match, mentions) {
          return choices
        ...
        delimiter: '#',
        findChoices: function findChoices(match, mentions) {
          return hashTags.filter(function (choice) {
            return ~('' + choice.tag).indexOf(match[1]);
          });
        },
        label: function label(item) {
          return item.tag;
        },
        highlight: function highlight(choice) {
...
var hashTags = [{ tag: 'yolo', id: 1 }, { tag: 'jpndts', id: 2 }, { tag: 'jklol', id: 3 }];
